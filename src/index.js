...
.stats-grid { display: grid; grid-template-columns: repeat(4, minmax(0, 1fr)); grid-auto-rows: 1fr; align-items: stretch; gap: 0; text-align: center; }
.stat-item { display: flex; flex-direction: column; align-items: center; gap: 8px; padding: 0 clamp(12px, 2.5vw, 36px); height: 100%; min-width: 0; }
.stat-item .stat-num { flex: 0 0 auto; }
.stat-item .stat-label { flex: 0 0 auto; min-height: 32px; }
.stat-item .stat-sub { flex: 1 1 auto; }
...
/* ── THREE PILLARS ── */
.section-pillars { padding: var(--section-pad) 0; position: relative; z-index: 2; background: var(--surface); }
.section-header { text-align: center; margin-bottom: 72px; }
.section-header h2 { font-family: var(--font-display); font-weight: 700; font-size: clamp(34px, 4vw, 56px); line-height: 1.18; letter-spacing: -0.015em; color: var(--ink); margin-bottom: 22px; max-width: 22ch; margin-left: auto; margin-right: auto; }
...
.teaser-h { font-family: var(--font-display); font-weight: 700; font-size: clamp(20px, 1.9vw, 26px); line-height: 1.25; letter-spacing: -0.01em; color: var(--ink); margin-bottom: 10px; }
...
.teaser-toggle { display: inline-flex; align-items: center; gap: 7px; margin-top: 20px; padding-top: 0; font-size: 11px; font-weight: 700; letter-spacing: 0.1em; text-transform: uppercase; color: var(--muted); cursor: pointer; border: none; background: none; padding-bottom: 6px; transition: color 0.2s; font-family: var(--font-body); align-self: flex-start; }
...
/* -- Luma mock (copied from dashboard.html coach panel) -- */
.luma-mock-card {
  background: #fff;
  border-radius: 20px;
...
.luma-mock-header {
  ...
  gap: 10px;
  padding: 14px 16px;
  background: linear-gradient(135deg, #154D33 0%, #1D6040 50%, #39BA76 100%);
  color: #fff;
  overflow: hidden;
}
.luma-mock-avatar-header {
  width: 32px;
  height: 32px;
  border-radius: 50%;
...
.luma-mock-title-wrap { display: flex; flex-direction: column; gap: 1px; }
.luma-mock-name { font-size: 15px; font-weight: 700; line-height: 1.2; }
.luma-mock-status {
  font-size: 11px;
  opacity: 0.85;
  display: flex;
...
.luma-mock-status-dot {
  width: 6px;
  height: 6px;
  border-radius: 50%;
  background: #8ef4b8;
  box-shadow: 0 0 8px #8ef4b8;
}
.luma-mock-messages {
  padding: 16px;
  display: flex;
...
.luma-mock-msg.user { align-self: flex-end; flex-direction: row-reverse; }
.luma-mock-msg.assistant { align-self: flex-start; }
.luma-mock-msg-avatar {
  width: 28px;
  height: 28px;
  border-radius: 50%;
  background: #154D33;
...
.luma-mock-msg.user .luma-mock-msg-avatar { display: none; }
.luma-mock-msg-bubble {
  padding: 10px 14px;
  border-radius: 16px;
  font-size: 14px;
  line-height: 1.5;
}
.luma-mock-msg.assistant .luma-mock-msg-bubble {
  background: #39BA76;
  color: #fff;
  border-bottom-left-radius: 4px;
}
.luma-mock-msg.user .luma-mock-msg-bubble {
  background: #F1F1F1;
  color: #1D6040;
...
.luma-section .mission span { color: #39BA76; }
.luma-section .intro,
.luma-section .subcopy {
  max-width: 620px;
  margin: 0 auto 14px;
  font-size: 15px;
  line-height: 1.6;
...
.luma-section .subcopy { margin-bottom: 48px; }
.luma-moments {
  display: flex;
  flex-direction: column;
  gap: 44px;
...
.luma-cta:hover { background: #1D6040; }
.luma-fade { opacity: 0; transform: translateY(8px); transition: opacity 0.25s ease-out, transform 0.25s ease-out; }
.luma-fade.in { opacity: 1; transform: translateY(0); }
@media (max-width: 480px) {
  .luma-section { padding: 60px 16px; }
  .luma-moments { gap: 36px; }
  .luma-mock-card { max-width: 100%; }
}

/* ── 24-HOUR TIMELINE (moved from /how-it-works) ── */
...
.t-luma-tile { padding: 0 !important; background: transparent !important; border: none !important; }
.t-luma-card { background: var(--white); border: 1.5px solid var(--border-mid); border-radius: 14px; overflow: hidden; display: flex; flex-direction: column; font-family: var(--font-body); }
.t-luma-card .mock-coach-context { display: flex; flex-wrap: wrap; gap: 6px; padding: 14px 16px 12px; border-bottom: 1px solid var(--border); }
.t-luma-card .mock-chip { font-size: 10px; font-weight: 700; letter-spacing: 0.04em; padding: 4px 10px; border-radius: 50px; background: var(--surface); color: var(--muted-2); border: 1px solid var(--border-mid); }
.t-luma-card .mock-chip.alert { background: rgba(234,88,12,0.07); color: #ea580c; border-color: rgba(234,88,12,0.18); }
.t-luma-card .mock-chip.low { background: rgba(239,68,68,0.07); color: #ef4444; border-color: rgba(239,68,68,0.18); }
.t-luma-card .mock-chat-area { display: flex; flex-direction: column; gap: 10px; padding: 14px 16px; }
.t-luma-card .mock-bubble { font-size: 12px; line-height: 1.55; padding: 10px 13px; border-radius: 10px; max-width: 90%; }
.t-luma-card .mock-bubble.user { align-self: flex-end; background: var(--surface); color: var(--muted-2); border-radius: 10px 10px 2px 10px; font-style: italic; }
.t-luma-card .mock-bubble.coach { align-self: flex-start; background: rgba(46,168,74,0.08); color: var(--ink); border-radius: 10px 10px 10px 2px; border: 1px solid rgba(46,168,74,0.15); }
.t-luma-card .mock-coach-label { font-size: 9px; font-weight: 800; letter-spacing: 0.14em; text-transform: uppercase; color: var(--green); margin-bottom: 5px; }
.t-luma-card .mock-input-row { display: flex; gap: 10px; align-items: center; padding: 12px 14px; border-top: 1px solid var(--border); }
.t-luma-card .mock-input-bar { flex: 1; height: 32px; background: var(--surface); border-radius: 8px; border: 1px solid var(--border); }
.t-luma-card .mock-send-btn { width: 32px; height: 32px; background: var(--green); border-radius: 8px; display: flex; align-items: center; justify-content: center; }
.t-luma-card .mock-send-btn svg { width: 14px; height: 14px; fill: none; stroke: white; stroke-width: 2; stroke-linecap: round; stroke-linejoin: round; }
...
/* ── Editorial h2 system (overrides legacy Bebas-era h2s on /) ── */
.timeline-header h2,
.testimonials-header h2 { font-family: var(--font-display) !important; font-weight: 700 !important; font-size: clamp(34px, 4vw, 56px) !important; line-height: 1.18 !important; letter-spacing: -0.015em !important; max-width: 22ch; margin-left: auto; margin-right: auto; }
.section-cta .cta-inner h2 { font-family: var(--font-display) !important; font-weight: 700 !important; font-size: clamp(36px, 5vw, 64px) !important; line-height: 1.12 !important; letter-spacing: -0.02em !important; max-width: 18ch; margin-left: auto; margin-right: auto; }

/* ── Eyebrow color: single source of truth ── */
.label, .label::before { color: var(--green); }
.section-cta .label { color: var(--green) !important; }
.section-cta .label::before { background: var(--green) !important; }
...
/* ── 3D TIMELINE JOURNEY ── */
.section-timeline { perspective: 1400px; perspective-origin: 50% 38vh; overflow: hidden; }
.section-timeline .timeline { transform-style: preserve-3d; }
.section-timeline .timeline::before {
  background: linear-gradient(to bottom, transparent, rgba(46,168,74,0.10) 8%, rgba(46,168,74,0.45) 50%, rgba(46,168,74,0.10) 92%, transparent);
  width: 2px;
  box-shadow: 0 0 20px rgba(46,168,74,0.25);
}
.section-timeline .timeline-item {
  transform-style: preserve-3d;
  will-change: transform, opacity;
  transform: translateZ(-260px) translateY(40px) scale(0.84);
  opacity: 0.22;
  transition: transform 0.7s cubic-bezier(0.2, 0.8, 0.2, 1), opacity 0.7s cubic-bezier(0.2, 0.8, 0.2, 1);
  /* override .reveal initial state. JS controls visibility */
  animation: none !important;
}
.section-timeline .timeline-item.reveal { opacity: 0.22; }
.section-timeline .timeline-item.tl-focus {
  transform: translateZ(0) translateY(0) scale(1);
  opacity: 1;
}
.section-timeline .timeline-item.tl-past {
  transform: translateZ(-160px) translateY(-20px) scale(0.9);
  opacity: 0.45;
}
.section-timeline .timeline-item .timeline-spacer { transform: translateZ(40px); }
.section-timeline .timeline-item .timeline-visual { transform: translateZ(20px); }
.section-timeline .timeline-item.tl-focus .timeline-visual {
  box-shadow: 0 1px 0 rgba(255,255,255,0.9) inset, 0 2px 4px rgba(20,40,30,0.05), 0 24px 48px rgba(20,40,30,0.10), 0 60px 120px rgba(57,186,118,0.10);
}
.section-timeline .timeline-item.tl-focus .timeline-node.active {
  box-shadow: 0 0 0 8px rgba(46,168,74,0.18), 0 0 28px rgba(46,168,74,0.45);
}

@media (prefers-reduced-motion: reduce) {
  .section-timeline { perspective: none; }
  .section-timeline .timeline-item { transform: none !important; opacity: 1 !important; transition: none !important; }
}

@media (max-width: 860px) {
  .section-timeline { perspective: 900px; perspective-origin: 50% 30vh; }
  .section-timeline .timeline-item { transform: translateZ(-160px) translateY(30px) scale(0.9); }
  .section-timeline .timeline-item.tl-past { transform: translateZ(-100px) scale(0.94); }
}

/* ── PRICING CARD UNIFORMITY ── */
.section-pricing .pricing-grid { align-items: stretch; }
.section-pricing .pricing-card { display: flex; flex-direction: column; }
.section-pricing .pricing-card.featured { transform: none; box-shadow: 0 0 0 2px var(--green), 0 24px 48px rgba(46,168,74,0.16), 0 4px 12px rgba(46,168,74,0.10); }
.section-pricing .pricing-card.featured:hover { transform: translateY(-6px); box-shadow: 0 0 0 2px var(--green), 0 32px 64px rgba(46,168,74,0.22), 0 8px 20px rgba(46,168,74,0.14); }
.section-pricing .pricing-features { flex: 1 1 auto; }
.section-pricing .pricing-card .btn { margin-top: auto; }

/* ── WORKS WITH STRIP ── */
.works-with { background: var(--white); border-top: 1px solid var(--border); border-bottom: 1px solid var(--border); padding: 14px 0; text-align: center; }
...
@media (max-width: 540px) {
  .ls-grid, .score-breakdown { grid-template-columns: 1fr !important; }
  .score-sub { display: none; }
  .score-main { font-size: clamp(56px, 18vw, 80px) !important; }
}

/* Mobile: pricing cards stack with recommended first */
@media (max-width: 768px) {
  .pricing-grid { display: flex !important; flex-direction: column !important; align-items: stretch; }
  .pricing-card.featured { order: -1; }
}

/* Mobile: pillar feature lists trim to 3 items */
@media (max-width: 540px) {
  .pillar-feature-list li:nth-child(n+4) { display: none; }
  .pillar-tabs { overflow-x: auto; -webkit-overflow-scrolling: touch; }
}

/* ── FAQ SECTION ── */
.section-faq { padding: var(--section-pad) 0; background: var(--surface); border-top: 1px solid var(--border); }
.faq-grid { max-width: 720px; margin: 0 auto; padding: 0 var(--gutter); display: flex; flex-direction: column; gap: 0; }
.faq-item { border-bottom: 1px solid var(--border); }
.faq-item:first-child { border-top: 1px solid var(--border); }
.faq-q { width: 100%; background: none; border: none; padding: 20px 0; text-align: left; font-size: 16px; font-weight: 600; color: var(--ink); cursor: pointer; display: flex; justify-content: space-between; align-items: center; gap: 16px; line-height: 1.4; }
.faq-q::after { content: '+'; font-size: 20px; font-weight: 400; color: var(--green); flex-shrink: 0; transition: transform 0.2s; }
.faq-q[aria-expanded="true"]::after { transform: rotate(45deg); }
.faq-a { display: none; padding: 0 0 20px; }
.faq-q[aria-expanded="true"] + .faq-a { display: block; }
.faq-a p { font-size: 15px; line-height: 1.7; color: var(--muted-2); margin: 0; }
</style>
</head>
<body>
...
<div class="mobile-nav" id="mobileNav">
  ...
  <div style="display:flex;flex-direction:column;align-items:center;gap:0;margin-top:8px;">
    ...
    <span style="font-size:12px;color:var(--muted);margin-top:8px;text-align:center;">No card needed. Free to try.</span>
  </div>
</div>
    ...
    <div class="hero-insight-card">
      ...
      <div class="hic-speedo-wrap">
        <svg class="hic-speedo-svg" viewBox="0 0 200 110">
          ...
          <text id="hic-speedo-num" x="100" y="86" text-anchor="middle" font-family="'Open Sans',system-ui,sans-serif" font-size="36" fill="#1a1a1a">0</text>
          <text x="100" y="100" text-anchor="middle" font-family="'Open Sans',system-ui,sans-serif" font-size="9" fill="rgba(26,26,26,0.38)" font-weight="600">/ 1000</text>
        </svg>
...
<div class="ticker" aria-hidden="true">
  <div class="ticker-track">
    <span class="ticker-item"><span class="ticker-dot"></span>HRV and Nutrition Connected</span>
    <span class="ticker-item"><span class="ticker-dot"></span>Longevity Patterns Revealed</span>
    <span class="ticker-item"><span class="ticker-dot"></span>Your Data. Your Answers.</span>
    <span class="ticker-item"><span class="ticker-dot"></span>25+ Wearables Synced</span>
    <span class="ticker-item"><span class="ticker-dot"></span>Food, Body and Mind as One System</span>
    <span class="ticker-item"><span class="ticker-dot"></span>Free to Try. No Card Needed.</span>
    <span class="ticker-item"><span class="ticker-dot"></span>Daily Insights from Your Own Data</span>
    <span class="ticker-item"><span class="ticker-dot"></span>Sleep, Stress and Recovery Tracked</span>
    <span class="ticker-item"><span class="ticker-dot"></span>Longevity Score Updated Weekly</span>
    <span class="ticker-item"><span class="ticker-dot"></span>Patterns That Change Everything</span>
    <span class="ticker-item"><span class="ticker-dot"></span>HRV and Nutrition Connected</span>
    <span class="ticker-item"><span class="ticker-dot"></span>Longevity Patterns Revealed</span>
    <span class="ticker-item"><span class="ticker-dot"></span>Your Data. Your Answers.</span>
    <span class="ticker-item"><span class="ticker-dot"></span>25+ Wearables Synced</span>
...
<!-- INTRO COMPOUND STRIP (reuses .section-compound/.compound-* with 4-col override) -->
<style>
.section-compound.compound-4 .compound-grid { grid-template-columns: repeat(4, minmax(0, 1fr)); max-width: 1480px; gap: 18px; }
@media (max-width: 980px) { .section-compound.compound-4 .compound-grid { grid-template-columns: repeat(2, 1fr); } }
@media (max-width: 540px) { .section-compound.compound-4 .compound-grid { grid-template-columns: 1fr; } }
</style>
<section class="section-compound compound-4">
  <div class="compound-grid">
    <div class="compound-eyebrow" style="text-align:center;margin-bottom:48px;">
      <h2 style="font-family:var(--font-display);font-weight:700;font-size:clamp(28px,3.2vw,42px);line-height:1.18;letter-spacing:-0.015em;color:var(--ink);max-width:34ch;margin:0 auto;">TLC uncovers the patterns that were hiding in plain sight.</h2>
    </div>
    <div class="compound-item reveal">
      <div class="compound-stat" style="white-space:nowrap;">Finally...</div>
      <div class="compound-h">You have the answers.</div>
      <p class="compound-body">Your meals, your wearable and how you actually felt. One adaptive system reading all three at once. The connections were always there. Now they show up.</p>
    </div>
    <div class="compound-item reveal d1">
      <div class="compound-stat" style="white-space:nowrap;">Day 1</div>
      <div class="compound-h">Tailored from the start.</div>
      <p class="compound-body">A five-minute onboarding sets your baseline. Your first insight is built around your data, your goals and your restrictions. Not a generic plan.</p>
    </div>
    <div class="compound-item reveal d2">
      <div class="compound-stat" style="white-space:nowrap;">Day 7</div>
      <div class="compound-h">The pattern appears.</div>
      <p class="compound-body">One week in, food, sleep and energy stop being separate inputs. They become one connected picture you can point to. Once you see it, you cannot unsee it.</p>
    </div>
    <div class="compound-item reveal d3">
      <div class="compound-stat" style="white-space:nowrap;">Day 30+</div>
      <div class="compound-h">You stop guessing.</div>
      <p class="compound-body">A month in, you are no longer reacting. You are anticipating. Your choices stop being guesswork and start being informed instinct.</p>
    </div>
  </div>
</section>
...
<!-- LUMA PROOF -->
<style>
.section-luma { padding: var(--section-pad) 0; background: var(--white); position: relative; z-index: 2; border-top: 1px solid var(--border); }
.luma-grid { display: grid; grid-template-columns: 1.15fr 1fr; gap: clamp(40px, 6vw, 88px); align-items: center; max-width: var(--content-max); margin: 0 auto; padding: 0 var(--gutter); }
.luma-copy .label { margin-bottom: 24px; color: var(--green); }
.luma-copy h2 { font-family: var(--font-display); font-weight: 700; font-size: clamp(30px, 3.4vw, 46px); line-height: 1.18; letter-spacing: -0.015em; color: var(--ink); margin-bottom: 18px; max-width: 18ch; }
.luma-copy p { font-size: 17px; line-height: 1.7; color: var(--muted-2); max-width: 46ch; margin-bottom: 28px; font-weight: 400; }
.luma-tiles { display: grid; grid-template-columns: 1fr 1fr; gap: 14px; margin-top: 8px; }
.luma-tile { background: var(--surface); border: 1px solid var(--border); border-radius: 14px; padding: 18px 18px; }
.luma-tile-eyebrow { font-size: 10px; font-weight: 700; letter-spacing: 0.16em; text-transform: uppercase; color: var(--muted); margin-bottom: 8px; display: flex; align-items: center; gap: 8px; }
.luma-tile-eyebrow::before { content: ''; width: 6px; height: 6px; border-radius: 50%; background: var(--green); }
.luma-tile.body-tile .luma-tile-eyebrow::before { background: var(--body-blue); }
.luma-tile.mind-tile .luma-tile-eyebrow::before { background: var(--mind); }
.luma-tile p { font-size: 14px; line-height: 1.55; color: var(--ink-2); margin: 0; font-weight: 400; }
.luma-mock { background: linear-gradient(160deg, #154D33 0%, #0d3621 100%); border-radius: 24px; padding: 28px; box-shadow: 0 30px 80px rgba(21,77,51,0.22), 0 8px 24px rgba(0,0,0,0.10); position: relative; overflow: hidden; }
.luma-mock::before { content: ''; position: absolute; inset: 0; background: radial-gradient(circle at 20% 0%, rgba(57,186,118,0.20) 0%, transparent 55%); pointer-events: none; }
.luma-mock-head { display: flex; align-items: center; gap: 12px; margin-bottom: 22px; position: relative; z-index: 1; }
.luma-mock-avatar { width: 38px; height: 38px; border-radius: 50%; background: linear-gradient(135deg, #39BA76, #1D6040); display: flex; align-items: center; justify-content: center; color: #fff; font-weight: 700; font-size: 14px; letter-spacing: 0.02em; flex-shrink: 0; }
.luma-mock-meta { display: flex; flex-direction: column; gap: 2px; }
.luma-mock-name { color: #fff; font-weight: 700; font-size: 14px; letter-spacing: 0.01em; }
.luma-mock-role { color: rgba(255,255,255,0.55); font-size: 11px; letter-spacing: 0.06em; text-transform: uppercase; font-weight: 600; }
.luma-mock-bubble { background: rgba(255,255,255,0.96); color: var(--ink); border-radius: 16px 16px 16px 4px; padding: 18px 20px; font-size: 15px; line-height: 1.6; position: relative; z-index: 1; max-width: 92%; box-shadow: 0 8px 24px rgba(0,0,0,0.08); font-weight: 400; }
.luma-mock-pattern { color: var(--green-dark); font-weight: 700; }
.luma-mock-stamp { color: rgba(255,255,255,0.55); font-size: 11px; letter-spacing: 0.06em; text-transform: uppercase; margin-top: 14px; font-weight: 600; position: relative; z-index: 1; }
@media (max-width: 860px) { .luma-grid { grid-template-columns: 1fr; gap: 36px; } .luma-tiles { grid-template-columns: 1fr; } .luma-copy h2 { max-width: 100%; } }
</style>
<section class="section-luma" id="luma-proof">
  <div class="luma-grid">
    <div class="luma-copy reveal">
      <span class="label" style="text-transform:none;letter-spacing:0.01em;font-size:14px;">Meet Luma, your longevity coach, always on and continuously reading the signals across your mind, body, and nutrition.</span>
      <h2>All day, your body has been sending signals.<br>Until now, they were easy to miss.</h2>
      <p>Luma is always with you, quietly processing everything. Every meal. Every signal. Every number.<br>The patterns were always there. Now they&rsquo;re unmistakable.<br><br>She observes, analyses, and optimises in real time.</p>
      <div class="luma-tiles">
        <div class="luma-tile body-tile">
          <div class="luma-tile-eyebrow">Activity to mind</div>
          <p>On weeks you trained four or more times, your reported focus rose 18%. The link your tracker cannot draw.</p>
        </div>
        <div class="luma-tile mind-tile">
          <div class="luma-tile-eyebrow">Mind to body</div>
          <p>On nights after a stressful day, your resting heart rate ran 6 bpm higher. Now it has a name.</p>
        </div>
      </div>
    </div>
    <div class="luma-mock luma-mock-live reveal d2" id="lumaInteractive">
      <div class="luma-mock-head">

        <div class="luma-mock-meta">
          <span class="luma-mock-name">Luma</span>
          <span class="luma-mock-role">Your longevity coach</span>
        </div>
        <div class="luma-mock-status"><span class="luma-mock-status-dot"></span>Live</div>
      </div>
      <div class="luma-mock-context">
        <span class="luma-chip alert">HRV: 41ms</span>
        <span class="luma-chip alert">Sleep: 6.2h</span>
        <span class="luma-chip low">Recovery: Low</span>
        <span class="luma-chip">Goal: Longevity</span>
      </div>
      <div class="luma-mock-chat" id="lumaChat" aria-live="polite"></div>
      <div class="luma-mock-suggestions">
        <button type="button" class="luma-suggest" data-script="0">Why am I dragging today?</button>
...
<style>
.luma-mock-status { margin-left: auto; display: inline-flex; align-items: center; gap: 6px; color: rgba(255,255,255,0.72); font-size: 11px; font-weight: 700; letter-spacing: 0.08em; text-transform: uppercase; padding: 4px 10px; background: rgba(255,255,255,0.06); border: 1px solid rgba(255,255,255,0.10); border-radius: 50px; position: relative; z-index: 1; }
.luma-mock-status-dot { width: 6px; height: 6px; border-radius: 50%; background: #6FE3A1; box-shadow: 0 0 0 3px rgba(111,227,161,0.18); animation: livePulse 2s ease-in-out infinite; }
.luma-mock-context { display: flex; flex-wrap: wrap; gap: 6px; margin-bottom: 14px; padding-bottom: 14px; border-bottom: 1px solid rgba(255,255,255,0.08); position: relative; z-index: 1; }
.luma-chip { font-size: 11px; font-weight: 700; letter-spacing: 0.02em; padding: 4px 10px; border-radius: 50px; background: rgba(255,255,255,0.08); color: rgba(255,255,255,0.78); border: 1px solid rgba(255,255,255,0.12); }
.luma-chip.alert { background: rgba(234,88,12,0.15); color: #FFB783; border-color: rgba(234,88,12,0.30); }
.luma-chip.low { background: rgba(239,68,68,0.15); color: #FF9B9B; border-color: rgba(239,68,68,0.30); }
.luma-mock-chat { display: flex; flex-direction: column; gap: 10px; min-height: 168px; position: relative; z-index: 1; padding: 4px 0 8px; }
.luma-bubble { font-size: 14.5px; line-height: 1.55; padding: 12px 16px; border-radius: 14px; max-width: 88%; opacity: 0; transform: translateY(8px); animation: lumaBubbleIn 0.45s cubic-bezier(0.2,0.8,0.2,1) forwards; }
@keyframes lumaBubbleIn { to { opacity: 1; transform: translateY(0); } }
.luma-bubble.user { align-self: flex-end; background: rgba(255,255,255,0.10); color: #fff; border-radius: 14px 14px 4px 14px; font-style: italic; }
.luma-bubble.luma { align-self: flex-start; background: rgba(255,255,255,0.96); color: var(--ink); border-radius: 14px 14px 14px 4px; box-shadow: 0 6px 20px rgba(0,0,0,0.10); }
.luma-bubble.luma strong { color: var(--green-dark); font-weight: 700; }
.luma-bubble.typing { background: rgba(255,255,255,0.96); border-radius: 14px 14px 14px 4px; padding: 14px 18px; align-self: flex-start; display: inline-flex; gap: 5px; align-items: center; }
.luma-bubble.typing span { width: 6px; height: 6px; border-radius: 50%; background: rgba(20,40,30,0.45); animation: lumaTypingDot 1.1s ease-in-out infinite; }
.luma-bubble.typing span:nth-child(2) { animation-delay: 0.18s; }
.luma-bubble.typing span:nth-child(3) { animation-delay: 0.36s; }
@keyframes lumaTypingDot { 0%,60%,100% { opacity: 0.3; transform: translateY(0); } 30% { opacity: 1; transform: translateY(-3px); } }
.luma-mock-suggestions { display: flex; flex-wrap: wrap; gap: 8px; margin-top: 14px; padding-top: 14px; border-top: 1px solid rgba(255,255,255,0.08); position: relative; z-index: 1; }
.luma-suggest { font-family: inherit; font-size: 12px; font-weight: 600; letter-spacing: 0.01em; color: rgba(255,255,255,0.85); background: rgba(255,255,255,0.06); border: 1px solid rgba(255,255,255,0.12); border-radius: 50px; padding: 7px 14px; cursor: pointer; transition: background 0.2s, border-color 0.2s, transform 0.15s; }
.luma-suggest:hover { background: rgba(232,184,75,0.16); border-color: rgba(232,184,75,0.45); color: #fff; transform: translateY(-1px); }
.luma-suggest.active { background: rgba(111,227,161,0.18); border-color: rgba(111,227,161,0.50); color: #fff; }
.luma-mock-closing { color: rgba(255,255,255,0.78); font-size: 13px; line-height: 1.55; margin-top: 18px; padding-top: 16px; border-top: 1px solid rgba(255,255,255,0.10); position: relative; z-index: 1; }
.luma-mock-link { color: #6FE3A1; font-weight: 700; text-decoration: none; letter-spacing: 0.01em; border-bottom: 1px solid rgba(111,227,161,0.35); padding-bottom: 1px; transition: color 0.2s, border-color 0.2s; white-space: nowrap; margin-left: 6px; }
.luma-mock-link:hover { color: #9EF0BD; border-bottom-color: rgba(158,240,189,0.7); }
@media (prefers-reduced-motion: reduce) { .luma-bubble { opacity: 1; transform: none; animation: none; } .luma-bubble.typing span { animation: none; opacity: 0.6; } .luma-mock-status-dot { animation: none; } }
</style>
...
<section class="section-timeline">
  <div class="container">
    <div class="timeline-header reveal">
      ...
      <h2>When everything connects, answers appear.</h2>
      <p>Five moments. One picture you have never been able to see before. This is what changes when your food, your body and your mind finally talk to each other.</p>
    </div>
    ...
    <div class="timeline">
      ...
      <div class="timeline-item left reveal d1">
        <div class="timeline-content">
          ...
          <h3>Lunch, anywhere.</h3>
          <p>You’re out for lunch. You open Meal Matchmaker. TLC already knows your calorie needs for the day, your lunch macros, goals, allergies, and taste preferences. It scans real menus from nearby eateries and finds the perfect match for you. </p>
          <p style="margin-top:12px;">Not a suggestion. A specific dish, from a real menu, with your actual macro targets checked. Every meal an informed decision, wherever you are. <strong>All meals, calories & macros logged automatically.</strong></p>
          <p style="margin-top:12px;">At home? TLC's Pantry Pal scans your fridge and cupboards, uses what you already have at home, and builds instant recipes tailored to you. Or follow a pre-made TLC meal plan designed a week in advance around your preferences.</p>
        </div>
        <div class="timeline-spacer">
      ...
      <div class="timeline-item left reveal d1">
        <div class="timeline-content">
          ...
          <h3>Sixty seconds that close the loop.</h3>
          ...
          <p style="margin-top:12px;">Without it, TLC has two of the three pillars. With it, the third reading lands and tonight's insight has somewhere to point. "Every time your omega-3 intake was above target, your evening energy was 7.8 or higher." That kind of pattern only exists because you took 60 seconds.</p>
        </div>
        <div class="timeline-spacer">
          <div class="timeline-node active"></div>
...
<!-- COMPOUNDING STATS STRIP -->
<style>
.section-compound { padding: var(--section-pad) 0; background: linear-gradient(180deg, var(--white) 0%, var(--surface) 100%); border-top: 1px solid var(--border); position: relative; z-index: 2; }
.compound-grid { max-width: 1180px; margin: 0 auto; padding: 0 var(--gutter); display: grid; grid-template-columns: repeat(3, 1fr); gap: clamp(20px, 3vw, 32px); align-items: stretch; }
.compound-eyebrow { grid-column: 1 / -1; text-align: center; margin-bottom: 12px; }
.compound-item { background: linear-gradient(180deg, #ffffff 0%, #f6f8f4 100%); border: 1px solid var(--border); border-radius: 18px; padding: 40px 44px; box-shadow: 0 1px 0 rgba(255,255,255,0.9) inset, 0 12px 28px rgba(20,40,30,0.06), 0 30px 64px rgba(20,40,30,0.05); display: flex; flex-direction: column; height: 100%; }
.compound-stat { font-family: var(--font-display); font-weight: 700; font-size: clamp(40px, 4.5vw, 56px); line-height: 1; letter-spacing: -0.02em; color: var(--green); margin-bottom: 14px; }
.compound-h { font-family: var(--font-display); font-weight: 700; font-size: clamp(20px, 2vw, 26px); line-height: 1.25; letter-spacing: -0.01em; color: var(--ink); margin-bottom: 12px; }
.compound-body { font-size: 15px; line-height: 1.7; color: var(--muted-2); font-weight: 400; }
@media (max-width: 860px) { .compound-grid { grid-template-columns: 1fr; } }
</style>

<!-- THREE PILLARS -->
<style>
.section-pillars .teasers-grid { grid-template-columns: 1fr 1fr 1fr; gap: 24px; }
.section-pillars .teaser-card.tc-mind { grid-column: 1 / -1; order: -1; padding: clamp(40px, 5vw, 64px) clamp(32px, 5vw, 72px); background: linear-gradient(135deg, #154D33 0%, #1D6040 60%, #2a7a4a 100%); border: 1px solid rgba(232,184,75,0.20); box-shadow: 0 1px 0 rgba(255,255,255,0.10) inset, 0 24px 48px rgba(20,40,30,0.18), 0 60px 120px rgba(232,184,75,0.10); display: flex; flex-direction: column; align-items: center; text-align: center; position: relative; overflow: hidden; isolation: isolate; max-width: 560px; width: 100%; margin-left: auto; margin-right: auto; aspect-ratio: 2/3; min-height: fit-content; }
.section-pillars .teaser-card.tc-mind > *:not(.thread-backdrop) { position: relative; z-index: 1; }
.thread-backdrop { position: absolute; inset: 0; pointer-events: none; z-index: 0; overflow: hidden; }
.thread-backdrop canvas { position: absolute; inset: 0; width: 100%; height: 100%; display: block; image-rendering: pixelated; image-rendering: -moz-crisp-edges; image-rendering: crisp-edges; }
.thread-backdrop::after { content: ''; position: absolute; inset: 0; background: linear-gradient(135deg, rgba(21,77,51,0.55) 0%, rgba(29,96,64,0.42) 50%, rgba(42,122,74,0.38) 100%); transition: background 1.8s ease-out; pointer-events: none; }
.section-pillars .teaser-card.tc-mind.thread-revealed .thread-backdrop::after { background: linear-gradient(135deg, rgba(21,77,51,0.42) 0%, rgba(29,96,64,0.30) 50%, rgba(42,122,74,0.26) 100%); }
.section-pillars .teaser-card.tc-mind::before { display: none; }
.section-pillars .teaser-card.tc-mind .teaser-icon { background: rgba(232,184,75,0.16); width: 104px; height: 104px; margin-bottom: 28px; border-radius: 22px; }
.section-pillars .teaser-card.tc-mind .teaser-icon svg { width: 56px; height: 56px; stroke: var(--mind); }
.section-pillars .teaser-card.tc-mind > div:nth-child(2) { color: var(--mind); margin-bottom: 14px; }
.section-pillars .teaser-card.tc-mind .teaser-h { color: #fff; font-size: clamp(18px, 2.2vw, 26px); line-height: 1.35; letter-spacing: -0.01em; margin-bottom: 16px; max-width: 32ch; text-align: center; font-weight: 600; }
.section-pillars .teaser-card.tc-mind .teaser-h + .teaser-h { margin-top: 0; }
.section-pillars .teaser-card.tc-mind .teaser-sub { color: rgba(232,184,75,0.85); font-style: normal; font-weight: 600; font-size: 13px; letter-spacing: 0.04em; text-transform: uppercase; margin-bottom: 18px; text-align: center; }
.section-pillars .teaser-card.tc-mind .teaser-body { color: rgba(255,255,255,0.80); font-size: 17px; line-height: 1.7; max-width: 60ch; text-align: center; margin-bottom: 22px; }
.section-pillars .teaser-card.tc-mind .teaser-toggle { color: rgba(255,255,255,0.55); align-self: center; }
.section-pillars .teaser-card.tc-mind .teaser-toggle:hover { color: rgba(255,255,255,0.85); }
.section-pillars .teaser-card.tc-mind .teaser-expandable { width: 100%; max-width: 720px; margin: 0 auto; text-align: left; }
.section-pillars .teaser-card.tc-mind .teaser-tool-name,
.section-pillars .teaser-card.tc-mind .teaser-prop-name { color: #fff; }
.section-pillars .teaser-card.tc-mind .teaser-tool-desc,
.section-pillars .teaser-card.tc-mind .teaser-prop-desc { color: rgba(255,255,255,0.65); }
.section-pillars .teaser-card.tc-mind .teaser-tool-list,
.section-pillars .teaser-card.tc-mind .teaser-proprietary { border-top-color: rgba(255,255,255,0.10); }
.section-pillars .teaser-card.tc-mind .teaser-prop-label { color: var(--mind); }
.section-pillars .teaser-card.tc-mind .teaser-prop-badge { background: rgba(232,184,75,0.18); color: var(--mind); }
.section-pillars .teaser-card.tc-mind .teaser-cta { color: var(--mind); border-bottom-color: rgba(232,184,75,0.4); margin-top: 14px; align-self: center; }
/* Mind pillar tile */
.section-pillars .teaser-card.tc-mind-tile::before { background: linear-gradient(90deg, var(--mind), #b89010); }
.section-pillars .teaser-card.tc-mind-tile .teaser-icon { background: rgba(232,184,75,0.10); }
.section-pillars .teaser-card.tc-mind-tile .teaser-icon svg { stroke: var(--mind); }
.section-pillars .teaser-card.tc-mind-tile .teaser-cta { color: #927010; border-bottom-color: rgba(232,184,75,0.3); }
.section-pillars .teaser-card.tc-mind-tile:hover .teaser-cta { border-bottom-color: var(--mind); }
/* Tile compaction */
.section-pillars .teaser-card.tc-di,
.section-pillars .teaser-card.tc-body,
.section-pillars .teaser-card.tc-mind-tile { padding: 32px 28px 28px; }
.section-pillars .teaser-card.tc-di .teaser-h,
.section-pillars .teaser-card.tc-body .teaser-h,
.section-pillars .teaser-card.tc-mind-tile .teaser-h { font-size: clamp(20px, 1.9vw, 24px); margin-bottom: 8px; }
.section-pillars .teaser-card.tc-di .teaser-body,
.section-pillars .teaser-card.tc-body .teaser-body,
.section-pillars .teaser-card.tc-mind-tile .teaser-body { font-size: 14px; }
@media (max-width: 720px) {
  .section-pillars .teasers-grid { grid-template-columns: 1fr; }
  .section-pillars .teaser-card.tc-mind { grid-column: auto; padding: 32px 24px; grid-template-columns: 1fr; gap: 18px; }
  .section-pillars .teaser-card.tc-mind > * { grid-column: auto !important; }
}
</style>
<section class="section-pillars" id="pillars">
  <div class="container">
    <div class="section-header reveal">
      <span class="label" style="justify-content:center;margin-bottom:20px;display:inline-flex;">The thread</span>
      <h2 style="text-align:center;">Three pillars. One unified picture.</h2>
      <p>Nutrition. Body. Mind. Most tools track one in isolation. TLC reads all three together, and the insight lives in the connection between them.</p>
    </div>
    <div class="teasers-grid">
      ...
      <div class="teaser-card tc-di reveal d1">
        <div class="teaser-icon">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.75" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M3 2v7c0 1.1.9 2 2 2h4a2 2 0 002-2V2"/><path d="M7 2v20"/><path d="M21 15V2a5 5 0 00-5 5v6c0 1.1.9 2 2 2h3zm0 0v7"/></svg>
        </div>
        ...
        <h3 class="teaser-h">What you eat, dialled in.</h3>
        <p class="teaser-sub">Meals aligned to your goals, your body, and your day.</p>
        <p class="teaser-body">Every choice guided by your goals, your body, and what's already in your fridge. Build meals in seconds or plan a full week with less waste and less guesswork.</p>
        ...
        <div class="teaser-expandable">
          ...
          <div class="teaser-proprietary">
            <div class="teaser-prop-label">Nutrition Intelligence</div>
      ...
      <div class="teaser-card tc-body reveal d2">
        <div class="teaser-icon">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.75" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><circle cx="12" cy="5" r="1"/><path d="m9 20 3-6 3 6"/><path d="m6 8 6 2 6-2"/><path d="M12 10v4"/></svg>
        </div>
        ...
        <h3 class="teaser-h">Your biometrics, made useful.</h3>
        <p class="teaser-sub">Data that shows you what to do next.</p>
        <p class="teaser-body">Sleep, recovery, activity, and energy in one place. TLC translates your wearable data into specific daily actions so you always know what to do next.</p>
        <button class="teaser-toggle" onclick="event.stopPropagation();event.preventDefault();toggleTeaser(this)">
          <span class="teaser-toggle-icon"><svg viewBox="0 0 10 10" width="10" height="10" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round"><line x1="5" y1="1" x2="5" y2="9"/><line x1="1" y1="5" x2="9" y2="5"/></svg></span>
          <span class="teaser-toggle-label">See what's included</span>
        </button>
        <div class="teaser-expandable">
          ...
          <div class="teaser-proprietary">
            <div class="teaser-prop-label">Proprietary metrics</div>
            <div class="teaser-prop-item">
      ...
      <!-- MIND -->
      <div class="teaser-card tc-mind-tile reveal d3">
        <div class="teaser-icon">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.75" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M12 5a3 3 0 1 0-5.997.125 4 4 0 0 0-2.526 5.77 4 4 0 0 0 .556 6.588A4 4 0 1 0 12 18Z"/><path d="M12 5a3 3 0 1 1 5.997.125 4 4 0 0 1 2.526 5.77 4 4 0 0 1-.556 6.588A4 4 0 1 1 12 18Z"/><path d="M15 13a4.5 4.5 0 0 1-3-4 4.5 4.5 0 0 1-3 4"/><path d="M12 18v-5.5"/></svg>
        </div>
        ...
        <h3 class="teaser-h">Your mind, understood.</h3>
        <p class="teaser-sub">What you feel, finally makes sense.</p>
        <p class="teaser-body">Daily reflections, mood patterns, and energy signals read together. TLC shows you what's beneath the surface and gives you the clarity to act on it.</p>
        <button class="teaser-toggle" onclick="event.stopPropagation();event.preventDefault();toggleTeaser(this)">
          <span class="teaser-toggle-icon"><svg viewBox="0 0 10 10" width="10" height="10" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round"><line x1="5" y1="1" x2="5" y2="9"/><line x1="1" y1="5" x2="9" y2="5"/></svg></span>
          <span class="teaser-toggle-label">See what's included</span>
        </button>
        <div class="teaser-expandable">
          <div class="teaser-tool-list">
            <div class="teaser-tool-item">
              <span class="teaser-tool-name">Evening Reflection</span>
              <span class="teaser-tool-desc">A 60-second check-in that tracks energy, mood, focus, digestion, and recovery. Your baseline for everything TLC learns about you.</span>
            </div>
            <div class="teaser-tool-item">
              <span class="teaser-tool-name">Luma Coach</span>
              <span class="teaser-tool-desc">Your longevity coach trained across nutrition, physiology, mental performance, and habit science. Always context-aware. Always in your corner.</span>
            </div>
          </div>
          <div class="teaser-proprietary">
            <div class="teaser-prop-label">Mind Intelligence</div>
            <div class="teaser-prop-item">
              <span class="teaser-prop-name">Food-Mood Correlation</span>
              <span class="teaser-prop-desc">Your unique patterns showing which foods drive your energy, focus, and mood. Built from your own data, not population averages.</span>
            </div>
            <div class="teaser-prop-item">
              <span class="teaser-prop-name">Mental Load Index</span>
              <span class="teaser-prop-desc">How stress and cognitive load interact with your sleep and nutrition. Spots the days you need recovery before your body tells you.</span>
            </div>
            <div class="teaser-prop-item">
              <span class="teaser-prop-name">Clarity Trend</span>
              <span class="teaser-prop-desc">Weekly trajectory of focus and mood relative to your sleep, meals, and activity. The signal behind how sharp you feel each day.</span>
            </div>
          </div>
        </div>
      ...
      <!-- CROSS-PILLAR HERO -->
      <div class="teaser-card tc-mind reveal d3" id="threadHero">
        <div class="thread-backdrop" aria-hidden="true"><canvas id="threadCanvas"></canvas></div>
        <div style="font-size:10px;font-weight:800;letter-spacing:0.18em;text-transform:uppercase;color:var(--mind);margin-bottom:8px;">The thread</div>
        <h3 class="teaser-h">When you look at the data as individual dots, everything feels noisy and unclear.</h3>
        <h3 class="teaser-h">Connect them, and patterns begin to emerge. What once felt fragmented resolves into a clear picture that was there all along.</h3>
        <p class="teaser-body">One night of bad sleep is just a data point. Three nights in a row, after late dinners, heading into a stressful week, alongside low magnesium intake. That is a pattern.<br><br>TLC brings these signals together. It reads how your mind and body respond, then quietly guides you toward what works, and away from what doesn’t.</p>
        <a href="/how-it-works" class="teaser-cta">See the thread in action &rarr;</a>
      </div>
...
<section class="section-testimonials">
  ...
  <div class="container">
    <div class="testimonials-header reveal">
      ...
      <h2>People who let TLC join the dots.</h2>
    ...
    <div class="testimonials-grid">
      <div class="testimonial-card reveal d1">
        ...
        <div class="t-author"><div class="t-avatar"><span class="t-avatar-letter">S</span></div><div><div class="t-name">Sarah M., 38, marathon runner, London</div><div class="t-device">Oura Ring &bull; Fat loss goal</div></div></div>
      </div>
      ...
      <div class="testimonial-card reveal d3">
        ...
        <div class="t-author"><div class="t-avatar"><span class="t-avatar-letter">P</span></div><div><div class="t-name">Priya L., 44, GP and mother of two, Sydney</div><div class="t-device">Garmin &bull; Gut health goal</div></div></div>
      </div>
    </div>
  </div>
</section>
...
<section class="section-pricing" id="pricing">
  <div class="container">
    <div class="section-header reveal" style="margin-bottom:64px;">
      ...
      <h2>Choose your path to longevity.</h2>
      <p>Your patterns are already in the data. They are waiting to be read. Free to try, no card needed.</p>
      <p style="max-width:560px;margin:16px auto 0;font-size:15px;color:var(--ink-2);line-height:1.75;">At worst, nothing changes and it costs you nothing to try. At best, you see patterns no single app could show you, feel better every day, and take control of your long-term health.<br><br>Make one of the most valuable investments you will make this year, in your mind and your body.</p>
    </div>
    <div class="pricing-grid">
      <div class="pricing-card featured reveal-scale d1">
...
<p style="font-size:12px;color:var(--muted);margin-top:8px;text-align:center;">30-day money-back guarantee. No card needed for Lite.</p>
      ...
      <div class="pricing-card reveal-scale d2">
        ...
        <div class="pricing-period">per month</div>
        <p class="pricing-desc">Everything in TLC, plus a weekly meal plan that quietly cuts your grocery bill, your food waste and your carbon footprint.</p>
        <ul class="pricing-features">
          ...
          <li><svg class="pricing-check" viewBox="0 0 24 24" fill="none" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M20 6L9 17l-5-5"/></svg>Weekly meal plan tailored to your goals and kitchen</li>
          <li><svg class="pricing-check" viewBox="0 0 24 24" fill="none" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M20 6L9 17l-5-5"/></svg>Zero-waste shopping list that overlaps ingredients across the week</li>
          <li><svg class="pricing-check" viewBox="0 0 24 24" fill="none" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M20 6L9 17l-5-5"/></svg>Around 16% off your weekly grocery spend</li>
          <li><svg class="pricing-check" viewBox="0 0 24 24" fill="none" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M20 6L9 17l-5-5"/></svg>Around 7kg CO&#8322;e saved each month versus a typical diet</li>
...
<section class="section-cta"><div class="container"><div class="cta-inner">
  <span class="label" style="justify-content:center;margin-bottom:28px;display:inline-flex;">Day 1</span>
...
<section class="section-faq" id="faq">
  <div class="container">
    <div class="section-header reveal">
      <h2 style="text-align:center;font-size:clamp(28px,3vw,40px);margin-bottom:48px;">Common questions</h2>
    </div>
    <div class="faq-grid">
      <div class="faq-item reveal">
        <button class="faq-q" aria-expanded="false">Does TLC work with my wearable?</button>
        <div class="faq-a"><p>TLC connects with WHOOP, Oura, Garmin, Fitbit, Withings, Polar, Suunto, and more. If you connect a wearable during onboarding, your biometric data is pulled automatically and feeds directly into your daily insight and Luma's coaching context.</p></div>
      </div>
      <div class="faq-item reveal">
        <button class="faq-q" aria-expanded="false">What if I don't have a wearable?</button>
        <div class="faq-a"><p>No wearable needed. At the end of each day, take two minutes to log your energy levels, sleep quality, mood, and meals. TLC reads these inputs exactly as it reads wearable data and surfaces the same patterns. Many members start this way and find the manual check-in itself becomes a useful daily habit. You can connect a wearable at any point once you are ready.</p></div>
      </div>
      <div class="faq-item reveal">
        <button class="faq-q" aria-expanded="false">Is my health data private?</button>
        <div class="faq-a"><p>Your data is never sold, never shared with third parties, and never used to train external models. It lives in your account and is used solely to generate your insights and coaching. You can export or delete it at any time.</p></div>
      </div>
      <div class="faq-item reveal">
        <button class="faq-q" aria-expanded="false">What happens after the free trial?</button>
        <div class="faq-a"><p>After 7 days you can continue on TLC Lite for free, or stay on your paid plan. No card is required for Lite. If you chose a paid plan, you are billed monthly and can cancel in one tap at any time. We also offer a 30-day money-back guarantee on all paid plans.</p></div>
      </div>
      <div class="faq-item reveal">
        <button class="faq-q" aria-expanded="false">How is TLC different from MyFitnessPal or WHOOP?</button>
        <div class="faq-a"><p>MyFitnessPal tracks calories. WHOOP tracks biometrics. Neither connects the two. TLC reads all three pillars -- nutrition, body, and mind -- simultaneously, and surfaces the patterns that only emerge when you look at all three together. No other platform does this.</p></div>
      </div>
      <div class="faq-item reveal">
        <button class="faq-q" aria-expanded="false">Can I cancel any time?</button>
        <div class="faq-a"><p>Yes. Cancel in one tap from your account settings. No penalties, no lock-in. You keep access until the end of your current billing period. TLC Lite remains free forever.</p></div>
      </div>
    </div>
  </div>
</section>
...
/* ── Thread-hero Mona Lisa pixelated zoom-out reveal ── */
(function setupThreadBackdrop() {
  var hero = document.getElementById('threadHero');
  ...
  // Crop focus: Mona Lisa's face is roughly centred horizontally, upper third vertically.
  // Source image natural size ~ 1200 x 1797.
  // Initial zoom-in crop = small box around the face. End crop = full painting (centred-cropped to card aspect).
  function getCropForProgress(p) {
    // p: 0 = max zoom (face only), 1 = full painting fit-to-card
    var nw = img.naturalWidth || 1200;
    var nh = img.naturalHeight || 1797;
    // Face centre in source pixels
    var fx = nw * 0.52;
    var fy = nh * 0.32;
    // Card aspect (we'll crop a rect with this aspect from the source)
    var cardAspect = canvas.clientWidth / Math.max(1, canvas.clientHeight);
    // Final crop: full painting
    var finalW = nw;
    var finalH = nh;
    // Initial crop: small zoom around face, matching painting aspect
    var initW = Math.min(nw, nh) * 0.10;
    var initH = initW * (nh / nw);
    // Interpolate
    var ease = p < 0 ? 0 : p > 1 ? 1 : p;
    // cubic ease-in: slow start, speeds up
    ease = Math.pow(ease, 3);
    var w = initW + (finalW - initW) * ease;
    var h = initH + (finalH - initH) * ease;
    // Centre on face at start, drift to full painting centre at end
    var cxFinal = nw / 2;
    var cyFinal = nh / 2;
    var cx = fx + (cxFinal - fx) * ease;
    var cy = fy + (cyFinal - fy) * ease;
    var sx = Math.max(0, Math.min(nw - w, cx - w / 2));
    var sy = Math.max(0, Math.min(nh - h, cy - h / 2));
    return { sx: sx, sy: sy, sw: w, sh: h };
  }

  function getResolutionForProgress(p) {
    // Effective canvas pixel resolution. Tiny at start, rising to native at end.
    // Visible card width ~ 600-1100 px desktop, 343 mobile. Cap at 480 for perf.
    var maxPx = Math.min(480, Math.max(180, canvas.clientWidth));
    var minPx = 14; // chunky abstract blocks
    var ease = p < 0 ? 0 : p > 1 ? 1 : p;
    ease = Math.pow(ease, 2);
    var w = Math.round(minPx + (maxPx - minPx) * ease);
    return w;
  }

  function draw(p) {
    if (!imgReady) { pendingDraw = p; return; }
    var cw = canvas.clientWidth;
    var ch = canvas.clientHeight;
    if (!cw || !ch) return;
    var resW = getResolutionForProgress(p);
    var aspect = cw / ch;
    var resH = Math.max(1, Math.round(resW / aspect));
    if (canvas.width !== resW || canvas.height !== resH) {
      canvas.width = resW;
      canvas.height = resH;
    }
    ctx.imageSmoothingEnabled = false;
    var crop = getCropForProgress(p);
    ctx.clearRect(0, 0, resW, resH);
    // Cover mode: scale to fill canvas fully (no bars), anchor top-centre
    var scaleX = resW / crop.sw;
    var scaleY = resH / crop.sh;
    var scale = Math.max(scaleX, scaleY);
    var dw = Math.round(crop.sw * scale);
    var dh = Math.round(crop.sh * scale);
    var dx = Math.round((resW - dw) / 2);
    var dy = 0;
    ctx.drawImage(img, crop.sx, crop.sy, crop.sw, crop.sh, dx, dy, dw, dh);
  }

  function onResize() {
    // Re-render at current progress using current card size.
  ...
  function start() {
    if (revealed) return;
    revealed = true;
    hero.classList.add('thread-revealed');
    if (reduceMotion) {
      currentProgress = 1;
      draw(1);
      return;
  ...
  img.onload = function() {
    imgReady = true;
    // Initial paint at progress 0 (heavy pixelation, face-zoom).
    draw(pendingDraw != null ? pendingDraw : currentProgress);
  };
  img.onerror = function() {
    // Fallback: paint a flat gradient so the card is not empty.
    canvas.width = 8; canvas.height = 8;
    ctx.fillStyle = '#1D6040';
    ctx.fillRect(0, 0, 8, 8);
  };
  // Initial paint placeholder (in case img is slow): set canvas to a tiny green block.
  canvas.width = 8; canvas.height = 8;
  ctx.fillStyle = '#154D33';
  ctx.fillRect(0, 0, 8, 8);
  ...
  function maybeStart() {
    if (revealed) return;
    setTimeout(function() {
      if (imgReady) start(); else img.addEventListener('load', start);
    }, 220);
  }
  ...
  if (!('IntersectionObserver' in window)) {
    if (isInView()) maybeStart();
    window.addEventListener('scroll', function onScroll() {
      if (isInView()) { maybeStart(); window.removeEventListener('scroll', onScroll); }
    }, { passive: true });
    return;
  }
  ...
  // Belt-and-braces: also poll once on scroll, in case IO is suppressed.
  var scrollHandler = function() {
    if (revealed) { window.removeEventListener('scroll', scrollHandler); return; }
    if (isInView()) { maybeStart(); window.removeEventListener('scroll', scrollHandler); }
  };
  window.addEventListener('scroll', scrollHandler, { passive: true });

  // If hero is already visible at script-init time, start now.
  if (isInView()) maybeStart();
})();
...
(function setupTooltips() {
  document.addEventListener('click', function(e) {
    ...
    document.querySelectorAll('.feat-info.is-active').forEach(function(el) {
      if (el !== info) el.classList.remove('is-active');
    });
    if (info) {
      e.preventDefault();
      e.stopPropagation();
...
/* ── Interactive Luma demo ── */
(function setupLumaDemo() {
  var root = document.getElementById('lumaInteractive');
  var chat = document.getElementById('lumaChat');
  if (!root || !chat) return;
  var SCRIPTS = [
    {
      user: 'Why am I dragging today?',
      luma: '<strong>Three nights this week you ate after 9 pm.</strong> Deep sleep dropped 22%. Tonight, eat before 7:45. We will watch tomorrow’s HRV together.'
    },
    {
      user: 'What should I eat tonight?',
      luma: 'Lean anti-inflammatory. The <strong>salmon dinner last Tuesday</strong> gave you your best recovery score this week. Leafy greens and walnuts will help. Avoid caffeine after 1 pm.'
    },
    {
      user: 'Should I train today?',
      luma: 'Your readiness sits at <strong>58, the lowest this fortnight.</strong> Skip the heavy lift. A 20-minute zone 2 walk will do more for tomorrow’s HRV than pushing through.'
    }
  ];
  var timers = [];
  ...
  function play(idx) {
    ...
    var step1 = reduced ? 0 : 220;
    var step2 = reduced ? 0 : 1100;
    var step3 = reduced ? 0 : 2400;
    timers.push(setTimeout(function() {
      var u = document.createElement('div');
      u.className = 'luma-bubble user';
      u.textContent = '“' + script.user + '”';
      chat.appendChild(u);
    }, step1));
    timers.push(setTimeout(function() {
    ...
    timers.push(setTimeout(function() {
      ...
      l.className = 'luma-bubble luma';
      l.innerHTML = script.luma;
      chat.appendChild(l);
    }, step3));
  }
  document.querySelectorAll('.luma-suggest').forEach(function(btn) {
    btn.addEventListener('click', function() {
      play(parseInt(btn.dataset.script, 10));
    });
  });
  if ('IntersectionObserver' in window) {
    var obs = new IntersectionObserver(function(entries) {
      entries.forEach(function(e) {
        if (e.isIntersecting && !root.dataset.played) {
...
/* ── 3D Timeline z-axis journey ── */
(function setupTimeline3D() {
  ...
  function update() {
    ...
    const focal = vh * 0.5;
    const focalRange = vh * 0.32;
    items.forEach(function(item) {
      const rect = item.getBoundingClientRect();
      const center = rect.top + rect.height / 2;
      const distance = Math.abs(center - focal);
      item.classList.remove('tl-focus', 'tl-past');
      if (distance < focalRange) {
        item.classList.add('tl-focus');
      } else if (center < focal) {
